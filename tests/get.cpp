...
BOOST_AUTO_TEST_CASE(tuple_at)
{
	...
	auto zero = Si::make_unique<staticdb::expressions::expression>(staticdb::expressions::literal(staticdb::values::make_unsigned_integer<std::uint8_t>(0)));
	...
	staticdb::values::value const result = staticdb::expressions::execute(first, root, staticdb::values::value(staticdb::values::unit()));
